...
void AllPixTimepixDigitizer::Digitize(){
  ...
  map<pair<G4int, G4int>, MC_content> pixelsContent_MC; //contains information with only MC //nalipour
  for(G4int itr  = 0 ; itr < nEntries ; itr++) {
	...
	for(int i=-nseek;i<=nseek;i++){
	  for(int j=-nseek;j<=nseek;j++){
		...
		G4cout << "*******************" << G4endl;
		G4cout << "xpos=" << xpos << ", ypos=" << ypos << ", zpos=" << zpos << ", sigma=" << sigma << G4endl;
		G4cout << "i=" << i << ", j=" << j << ", hit_energy=" << eHit << ", Etemp=" << Etemp << G4endl;
  ...
  for( ; pCItr != pixelsContent.end() ; pCItr++)
      ...
      if(((*pCItr).second) > ThresholdMatrix[x][y])
	{
	  ...
	  digit->SetPixelIDX(x);
	  digit->SetPixelIDY(y);
	  digit->SetPixelEnergyDep(((*pCItr).second)/keV); //Energy with charge sharing //nalipour
