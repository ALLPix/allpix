...
void AllPixTimepixDigitizer::Digitize(){
  ...
  for(G4int itr  = 0 ; itr < nEntries ; itr++) {
    ...
    if(debug) G4cout << "eHitTotal=" << eHitTotal << ", tempPixel.first=" << tempPixel.first << ", tempPixel.second=" << tempPixel.second << ", xpos=" << xpos << ", ypos=" << ypos << G4endl; 
    
    G4cout << "Nilou: eHitTotal=" << (*hitsCollection)[itr]->GetEdep() << ", tempPixel.first=" << tempPixel.first << ", tempPixel.second=" << tempPixel.second << ", xpos=" << xpos << ", ypos=" << ypos << G4endl;
	...
	//  G4cout << "Threshold" << SurrogateD[x][y] << endl;
	 // G4cout << "Energy= "<< (*pCItr).second/keV << endl;
	  ...
	  if(EnergyToTOTSurogate((*pCItr).second,x,y)>0){
		  ...
		  //  Again, one per pixel.
		  //nalipour
		  G4cout << "[Nilou: AllPixTimepixDigitizer]: x=" << x << ", y=" << y << ", energy=" << (*pCItr).second/keV << G4endl;
