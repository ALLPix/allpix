 ...
 */
AllPixRun::AllPixRun(AllPixDetectorConstruction * det, TString ofp, TString dataset, TString tempDir, G4bool writeFlag){
  ...
  if(!m_hitsCollection){
	...
	for (G4int itrCol = 0 ; itrCol < nHC ; itrCol++) {
		...
		for (G4int i = 0 ; i < NbHits ; i++) {
			...
			// This quantity is the same for every hit
			m_storableHits[itrCol]->kinEParent = (*m_hitsCollection)[i]->GetKinEParent();
