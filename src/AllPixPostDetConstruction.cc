...
AllPixPostDetConstruction::AllPixPostDetConstruction()
{
// Ctor.
...
//______________________________________________________________________________
AllPixPostDetConstruction *AllPixPostDetConstruction::GetInstance()
{
// Returns self pointer.
   if (fgInstance) return fgInstance;
   fgInstance = new AllPixPostDetConstruction();
   return fgInstance;
}
   
//______________________________________________________________________________
void AllPixPostDetConstruction::NewTrack(Double_t x, Double_t y, Double_t z)
{
// Add a new track and starting point.
   fCurrent = new TPolyLine3D();
   fTracks->Add(fCurrent);
   fCurrent->SetLineColor(kBlue);
//   fCurrent->SetLineWidth(2);
   AddPoint(x,y,z);
}   

//______________________________________________________________________________
void AllPixPostDetConstruction::AddPoint(Double_t x, Double_t y, Double_t z)
{
 
  // Add a new point on the current track.
  if (!fCurrent) return;

  if(fabs(x/um) < 300. && fabs(y/um) < 300. && fabs(z/um) < 300.)
     fCurrent->SetNextPoint(x/um, y/um, z/um);
}

//______________________________________________________________________________
void AllPixPostDetConstruction::WriteTracks(const char *filename)
{
...
//______________________________________________________________________________
//void AllPixPostDetConstruction::Initialize(TG4RootDetectorConstruction *dc)
void AllPixPostDetConstruction::Initialize()
{
  G4cout << "AllPixPostDetConstruction::Initialize() ..." << G4endl;
