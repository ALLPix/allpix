...
AllPixTruthWithLorentzAndTOTDigitizer::AllPixTruthWithLorentzAndTOTDigitizer(G4String modName, G4String hitsColName, G4String digitColName) 
...
AllPixTruthWithLorentzAndTOTDigitizer::~AllPixTruthWithLorentzAndTOTDigitizer(){

}

void AllPixTruthWithLorentzAndTOTDigitizer::Digitize(){

	// create the digits collection
	m_digitsCollection = new AllPixTruthWithLorentzAndTOTDigitsCollection("AllPixTruthWithLorentzAndTOTDigitizer", collectionName[0] );
	...
	for( ; pCItr != pixelsContent.end() ; pCItr++)
	{
		...
		if((*pCItr).second > m_digitIn.thl) // over threshold !
		{
			...
			AllPixTruthWithLorentzAndTOTDigit * digit = new AllPixTruthWithLorentzAndTOTDigit;
