...
class JudithData:
    ...
    def __init__(self,filename,plane=0):
        
        self.m_RootFile=TFile(filename)
        self.m_RawDataTree = self.m_RootFile.Get("Plane%i/Hits"%plane)
...
#             print data        
    ...
    def ClusterEvent(self,i,method="QWeighted",sigma=0.003,sigmaGC=0.003,sigmaPbPC=0.003):
        ...
        row_tmp = [s for s in self.m_RawDataTree.PixX]
        col_tmp = [s for s in self.m_RawDataTree.PixY]
        tot_tmp = [s for s in self.m_RawDataTree.Value]
...
#########################################################################################
def usage():
    print 'Converts FEI4 Telescope generated ROOT files to a clustered ROOT file (pyEudetNTuple format) https://github.com/pyEudetAnalysis/pyEudetAnalysis'
    print 'Usage:\n python %s <input ROOT file> <output ROOT file> <sensor plane number>' % ( sys.argv[0] )
...
if __name__ == "__main__":
    ...
    data = JudithData(filename,plane)
